    ...
    <div class="entry-comments" id="comments"><?php
            ...
            if (have_comments ()) { ?>
                <div class="comment-title"><h4 class="np-title-line">
                    <?php printf(
                        _n(esc_html__('1 comment', ''),
                        '%1$s ' . esc_html__('comments', ''),
                        get_comments_number()),
            ...
            $fields[ 'cancel_reply_link' ] = esc_html__('cancelreply取消回复', 'xintheme');
						$fields[ 'comment_notes_before' ]=$fields[ 'comment_notes_after' ] = '';
            $fields[ 'label_submit' ] = esc_html__('reply提交', 'xintheme');
            $fields[ 'comment_field' ] = 
                '<p class="comment-form-comment">'.
                    '<textarea name="comment" required="required" placeholder="'.esc_html__('Enter输入...', 'xintheme').'" id="comment" class="required" rows="7" tabindex="4"></textarea>'.
                '</p>';
            $fields[ 'title_reply' ] = esc_html__('comment', 'theme');
